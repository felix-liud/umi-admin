...
const VERIFY_CODE_PATH: string = `${API_PREFIX}/user/captcha`;
...
export interface LoginItemType {
  ...
  VerifyCode: React.FC<WrappedLoginItemProps>;
...
class WrapFormItem extends Component<LoginItemProps, LoginItemState> {
  ...
  constructor(props: LoginItemProps) {
    ...
    this.state = {
      count: 0,
      imgVerifyCodeUrl: VERIFY_CODE_PATH,
    };
  }
  ...
  getImgVerifyCode = () => {
    this.setState({
      imgVerifyCodeUrl: `${VERIFY_CODE_PATH}?version=${new Date().getTime()}`
    })
  }
  ...
  render() {
    ...
    if (type === 'VerifyCode') {
      const inputProps = omit(otherProps, ['onGetCaptcha', 'countDown']);
      const { imgVerifyCodeUrl } = this.state;
      return (
        <FormItem style={{marginBottom: 0}}>
          <Row gutter={8}>
            ...
            <Col span={8}>
              <img src={imgVerifyCodeUrl}
                alt="图片加载失败" 
                className={styles.getCaptcha}
                onClick={this.getImgVerifyCode}
              />
            </Col>
          </Row>
        </FormItem>
